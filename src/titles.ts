...
import { complete, type SummaryProvider } from "./llm";
...
export async function generateTitle(
  ...
  const text = await complete(prompt, provider ?? { type: "anthropic", model: TITLE_MODEL });
...
export async function titleSession(
  ...
  const title = await generateTitle(row.md, provider);
...
export async function backfillTitles(db: Database, opts: { limit?: number; provider?: SummaryProvider; onProgress?: (done: number, total: number, title: string) => void } = {}): Promise<{ generated: number; skipped: number }> {
  const rows = db.query(
  ...
  for (let i = 0; i < rows.length; i++) {
    try {
      const t = await titleSession(db, rows[i]!.id, opts.provider);
